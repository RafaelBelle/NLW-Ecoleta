...
const CreatePoint = () => {
    ...
    useEffect(() => {
        axios.get<IBGEUFResponse[]>('https://servicodados.ibge.gov.br/api/v1/localidades/estados?orderBy=nome').then(response => {
            setUFs(response.data.map(x => x.sigla));
        });
    }, []);
    ...
    useEffect(() => {
        ...
        axios.get<IBGECityResponse[]>(`https://servicodados.ibge.gov.br/api/v1/localidades/estados/${selectedUf}/municipios?orderBy=nome`).then(response => {
            setCities(response.data.map(x => x.nome));
        });
    }, [selectedUf]);
    ...
    function handleSelectionUf(e: ChangeEvent<HTMLSelectElement>){
        const uf = e.target.value;

        setSelectedUf(uf);
    }

    function handleSelectionCity(e: ChangeEvent<HTMLSelectElement>){
    ...
    function handleSelectItem(id: number){
        const alreadySeleted = selectedItems.findIndex(x => x === id);

        if(alreadySeleted >= 0){
            const filteresItems = selectedItems.filter(x => x !== id);
        ...
        <div id="page-create-point">
            ...
            <form onSubmit={handleSubmit}>
                ...
                <ul className="items-grid">
                    {items.map(x => (
                        <li key={x.id} onClick={() => handleSelectItem(x.id)} className={selectedItems.includes(x.id) ? 'selected' : ''}>
